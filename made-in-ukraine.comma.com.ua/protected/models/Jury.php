...
/**
 * This is the model class for table "comma_jury".
 *
...
class Jury extends BaseModel {
	...
	const    PATH_IMAGE_SRC = '/graphics/jury/src/',
		PATH_ICON_SRC = '/graphics/jury/icon/src/',
		IMAGE_326x326 = '/graphics/jury/326x326/',
		IMAGE_490x650 = '/graphics/jury/490x650/',
		IMAGE_652x326 = '/graphics/jury/652x326/';
	...
	public function thumbnailsRules() {
		return array(
			'image' => array(
				'326x326' => array('method' => 'crop', 'canPinch' => true, 'path' => self::IMAGE_326x326),
				'652x326' => array('method' => 'crop', 'canPinch' => true, 'path' => self::IMAGE_652x326)
			),
			'icon' => array(
				'490x650' => array('method' => 'crop', 'canPinch' => true, 'path' => self::IMAGE_490x650),
			)
		);
	}
	...
	public function tableName() {
		return parent::tablePrefix() . 'jury';
	}
	...
	public function rules() {
		...
		return array(
			...
			created_by, modified_by, blocked, added_time, edited_time, active', 'safe'),
		);
	}
	...
	public static function getSectionUrl() {
		...
		if (self::$SectionUrl == '') {
			...
			self::$SectionUrl .= '/' . Base::findControllerAlias('C_jury');
		}
	...
	protected function afterSave() {
		...
		if ($_FILES[__CLASS__]['tmp_name']['image'] != null) {
			...
			if ($doc) {
				...
				$Image->crop(array(326, 326))->save($_SERVER['DOCUMENT_ROOT'] . self::IMAGE_326x326 . $filename);
				$this->image_filename = basename($Image->crop(array(652, 326))->save($_SERVER['DOCUMENT_ROOT'] . self::IMAGE_652x326 . $filename));
		...
		if ($_FILES[__CLASS__]['tmp_name']['icon'] != null) {
			...
			if ($doc) {
				...
				$this->icon_filename = basename($Image->crop(array(400, 500))->save($_SERVER['DOCUMENT_ROOT'] . self::IMAGE_490x650 . $filename));
