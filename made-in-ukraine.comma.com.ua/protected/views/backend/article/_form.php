...
?>
	...
	<button class="btn btn-primary save-btn"><?php echo ($model->isNewRecord ? 'Добавить' : 'Сохранить') ?></button> 
 	<?php if(!$model->isNewRecord): ?>
        <button id="show-preview" class="btn btn-warning" target="_blank">Предпросмотр</button>
         <!--<a target="_blank" class="btn btn-warning" href="<?=$model->getItemUrl()?>">Предпросмотр</a> --> 
    <?php endif;?>
        ...
        <div class="tab-content">
        	...
        	<div class="tab-pane fade in active" id="tabs-info" style="height1: 400px;">
				...
				<div class="block admin-image">
					...
					<?php if($this->getAction()->id == 'update' && !empty($model->image_filename) && file_exists($_SERVER['DOCUMENT_ROOT'].Article::PATH_IMAGE.$model->image_filename) ):
				               
				        echo CHtml::image(Article::PATH_IMAGE.$model->image_filename).'<br />';  
			    	...
			    	<?php */ // $form->fileField($model,'image'); ?>
				...
				<div class="block admin-image">
					...
					<?php if($this->getAction()->id == 'update' && !empty($model->gif_filename) && file_exists($_SERVER['DOCUMENT_ROOT'].Article::PATH_IMAGE_GIF.$model->gif_filename) ):
				               
				        echo CHtml::image(Article::PATH_IMAGE_GIF.$model->gif_filename).'<br />';  
                ...
                <div class="block admin-image">
                    <?php echo $form->label($model,'icon_filename'); ?>

                    <?php if($this->getAction()->id == 'update' && !empty($model->icon_filename) && file_exists($_SERVER['DOCUMENT_ROOT'].Article::PATH_ICON.$model->icon_filename) ):

                        echo CHtml::image(Article::PATH_ICON.$model->icon_filename).'<br />';

                        echo $form->checkBox($model, 'icon_delete');
                        echo $form->labelEx($model,'icon_delete', array('style'=>'display: inline-block;')).'<br /><br />';
                        echo $form->error($model,'icon_delete');
                    else:
                        echo CHtml::image('http://placehold.it/450x280').'<br />';
                    endif; ?>

                    <?php // $form->fileField($model,'icon'); ?>
                </div>

                <div class="block admin-image">
                    <?php echo $form->label($model,'shareimage'); ?>

                    <?php if($this->getAction()->id == 'update' && !empty($model->share_image) && file_exists($_SERVER['DOCUMENT_ROOT'].Article::PATH_SHARE_IMAGE.$model->share_image) ):

                        echo CHtml::image(Article::PATH_SHARE_IMAGE.$model->share_image).'<br />';
		        ...
		        */ ?>
				...
				<div class="block">
		            ...
		            <?php echo $form->dropDownList($model ,'rubric_id', 
		                                            Rubrics::getItemsList(),  
		                                            array('data-placeholder' => 'Выберите рубрику', 'empty' => array( 0 => 'Выберите рубрику'), 'class' => '')); ?>
				...
				<div class="block">
					...
					<?php echo $form->checkBox($model,'light',array('class' => 'ibutton')); ?>
					<?php echo $form->error($model,'light'); ?>
				</div>

				<div class="block">
					<?php echo $form->labelEx($model,'on_main'); ?>
					<?php echo $form->checkBox($model,'on_main',array('class' => 'ibutton')); ?>
					<?php echo $form->error($model,'on_main'); ?>
				</div>

				<div class="block">
					<?php echo $form->labelEx($model,'interview'); ?>
					<?php echo $form->checkBox($model,'interview',array('class' => 'ibutton')); ?>
		    ...
		    <div class="tab-pane fade" id="tabs-tags" style="min-height: 400px;">
                <?php $tagsItems = ArticleHasTegs::model()->withArticle($model->id)->findAll();
	                        ...
	                        <?php if(count($this->languageList) > 0): ?>
	                            ...
	                            <div class="row">  
	                                ...
	                                <?php foreach ($this->languageList as $lang_id=>$lang_name): 
	                                    ...
	                                    $tagsTransferItems = TagsTransfer::model()->orderByNameDesc()->findAll('parent_id = :parent_id AND language_id = :language_id', 
	                                ...
	                                <?php endforeach;?>
	                                <button type-data="article-tag" data-id="<?=$HasTags->id?>" class="del-tag btn btn-danger btn-mini" href="#">Удалить</button>
	                            </div>
		...
		<?php if(count($this->languageList) > 0): ?>
				...
				?>
					...
					<div class="tab-pane fade <?=$activeClass;?>" id="tabs-<?=$lang_id;?>">
						...
						<div class="row-elements">
							<div style=" width: 48%;">
								<?php echo $form->label($model_transfer,'annotation'); ?>
								<?php echo $form->textArea($model_transfer,'annotation['.$lang_id.']',array('rows'=>10, 'cols'=>60 , 'maxlength'=>255,   'value' => $model_transfer->annotation )); ?>
								<?php echo $form->error($model_transfer,'annotation'); ?>
							</div>
