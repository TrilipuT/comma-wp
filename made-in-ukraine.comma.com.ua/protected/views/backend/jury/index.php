...
<?php else: ?> 
	...
	<div class="content noPad">
	    <table class="responsive table table-bordered" >
	        <thead>
	          <tr>
	            <th width="1%"><?=Jury::model()->getAttributeLabel('id');?></th>
	            <th><?=JuryTransfer::model()->getAttributeLabel('name');?></th>
	            <th><?=Jury::model()->getAttributeLabel('code_name');?></th>
	            <th width="1%"><?=Jury::model()->getAttributeLabel('active');?></th>
	            <th width="5%">Jury</th>
	        ...
	        <tbody id="sortable" >
					<?php foreach($list as $item):
						$JuryTransfer = JuryTransfer::model()->find('parent_id = :parent_id AND language_id = 2', array(':parent_id' => $item->id));
						?>
