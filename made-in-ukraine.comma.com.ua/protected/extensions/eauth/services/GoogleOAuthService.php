...
class GoogleOAuthService extends EOAuth2Service {
	...
	protected function fetchAttributes() {
		...
		if (!empty($info['picture']))
			$this->attributes['file_photo'] = $info['picture'];
 		
 		$this->attributes['birthday'] = $info['birthday']; // format: 0000-00-00
	...
	protected function getCodeUrl($redirect_uri) {
		...
		$newUrl = $parse_url['scheme'].'://'.$parse_url['host'].$parse_url['path'].'?'.$query[0];

		$url = parent::getCodeUrl($newUrl);

		//var_dump($newUrl, $url); exit;
