...
class FacebookOAuthService extends EOAuth2Service {
	...
	//	$info = (object)$this->makeSignedRequest('https://graph.facebook.com/me?fields=id,name,link,picture,username,birthday,hometown');
        $info = (object)$this->makeSignedRequest('https://graph.facebook.com/me?fields=id,name,link,picture,birthday,hometown');

		$this->attributes['id'] = $info->id;
		$this->attributes['name'] = $info->name;
		$this->attributes['url'] = $info->link;
		$this->attributes['nick'] = $info->username;
		$this->attributes['file_photo'] = $info->picture->data->url;

		$this->attributes['town'] = $info->hometown->name;
		if ($info->birthday != null) { // MM/DD/YYYY
			$birthday = explode('/', $info->birthday);
			$this->attributes['birthday'] = $birthday[2] . '-' . $birthday[0] . '-' . $birthday[1];
		}
	...
	protected function getCodeUrl($redirect_uri) {
		...
		$redirect_uri = 'http://' . $_SERVER['HTTP_HOST'] . '/login?service=facebook';
