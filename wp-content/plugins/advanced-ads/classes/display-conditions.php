...
class Advanced_Ads_Display_Conditions {
    ...
    <select name="<?php echo $name; ?>[operator]">
		...
		foreach ( $authors as $_author ) {
			...
			?>"><?php echo $_author->display_name; ?></label><input type="checkbox"
                                                                    id="advads-conditions-<?php echo $index; ?>-<?php echo $_author->ID; ?>"
                                                                    name="<?php echo $name; ?>[value][]" <?php checked( $_val, 1 ); ?>
                                                                    value="<?php echo $_author->ID; ?>"><?php
		}
		?>
        <p class="advads-conditions-not-selected advads-error-message"><?php _ex( 'Please select some items.', 'Error message shown when no display condition term is selected', 'advanced-ads' ); ?></p>
        </div>
		<?php if ( count( $authors ) >= $max_authors ) : ?><p
                class="advads-error-message"><?php printf( __( 'Only %d elements are displayed above. Use the <code>advanced-ads-admin-max-terms</code> filter to change this limit according to <a href="%s" target="_blank">this page</a>.', 'advanced-ads' ), $max_authors, ADVADS_URL . 'codex/filter-hooks//#utm_source=advanced-ads&utm_medium=link&utm_campaign=author-term-limit' ); ?></p><?php endif;
	}
    ...
    <select name="<?php echo $name; ?>[operator]">
        ...
        <div class="advads-conditions-single advads-buttonset advads-conditions-postid-buttons"><?php
	...
	public static function modify_post_search_sql( $sql ) {
		...
		$sql = preg_replace_callback( "/{$wpdb->posts}.post_(content|excerpt)( NOT)? LIKE '%(.*?)%'/", array(
			'Advanced_Ads_Display_Conditions',
			'modify_post_search_sql_callback'
		), $sql );
	...
	public function register_conditions() {
		$conditions = array(
			'posttypes' => array(// post types condition
				...
				'metabox'     => array( 'Advanced_Ads_Display_Conditions', 'metabox_post_type' ),
				// callback to generate the metabox
				'check'       => array( 'Advanced_Ads_Display_Conditions', 'check_post_type' ),
				// callback for frontend check
			...
			'postids'   => array(// post id condition
				...
				'metabox'     => array( 'Advanced_Ads_Display_Conditions', 'metabox_post_ids' ),
				// callback to generate the metabox
				'check'       => array( 'Advanced_Ads_Display_Conditions', 'check_post_ids' ),
				// callback for frontend check
			),
			'general'   => array(// general conditions
				'label'   => __( 'general conditions', 'advanced-ads' ),
				// 'description' => __( 'Choose on which individual posts, pages and public post type pages you want to display or hide ads.', 'advanced-ads' ),
				'metabox' => array( 'Advanced_Ads_Display_Conditions', 'metabox_general' ),
				// callback to generate the metabox
				'check'   => array( 'Advanced_Ads_Display_Conditions', 'check_general' ),
				// callback for frontend check
			),
			'author'    => array(// author conditions
				'label'   => __( 'author', 'advanced-ads' ),
				// 'description' => __( 'Choose on which individual posts, pages and public post type pages you want to display or hide ads.', 'advanced-ads' ),
				'metabox' => array( 'Advanced_Ads_Display_Conditions', 'metabox_author' ),
				// callback to generate the metabox
				'check'   => array( 'Advanced_Ads_Display_Conditions', 'check_author' ),
				// callback for frontend check
			),
		);
		...
		foreach ( $taxonomies as $_tax ) :
			...
			$conditions[ 'taxonomy_' . $_tax->name ] = array(
				...
				'metabox'  => array( 'Advanced_Ads_Display_Conditions', 'metabox_taxonomy_terms' ),
				// callback to generate the metabox
				'check'    => array( 'Advanced_Ads_Display_Conditions', 'check_taxonomies' ),
				// callback for frontend check
				'taxonomy' => $_tax->name,
				// unique for this type: the taxonomy name
			);
			...
			$conditions[ 'archive_' . $_tax->name ] = array(
				...
				'metabox'  => array( 'Advanced_Ads_Display_Conditions', 'metabox_taxonomy_terms' ),
				// callback to generate the metabox
				'check'    => array( 'Advanced_Ads_Display_Conditions', 'check_taxonomy_archive' ),
				// callback for frontend check
				'taxonomy' => $_tax->name,
				// unique for this type: the taxonomy name
			);
