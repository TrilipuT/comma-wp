...
class Advanced_Ads_Group {
	...
	protected function load_additional_attributes() {
		...
		if ( ! isset( $all_groups[ $this->id ] ) || ! is_array( $all_groups[ $this->id ] ) ) {
			return;
		}

		if ( isset( $this->ad_args['change-group'] ) ) {
			// some options was provided by the user
			$group_data = Advanced_Ads_Utils::merge_deep_array( array(
				$all_groups[ $this->id ],
				$this->ad_args['change-group']
			) );
		} else {
	...
	private function load_all_ads() {
		...
		$args = array(
			...
			'term'           => $this->slug,
			'orderby'        => 'id'
			// might want to avoid sorting as not needed for most calls and fast in PHP; slight I/O blocking concern
		);
