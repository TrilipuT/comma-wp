...
?>
    ...
    <div id="advads-visitor-conditions"><?php
		...
		?>
        <table class="advads-conditions-table">
            <tbody><?php
			if ( isset( $options ) ) :
				$i = 0;
				foreach ( $options as $_options ) :
					...
					if ( method_exists( $metabox[0], $metabox[1] ) ) {
						if ( $i > 0 ) :
							?>
                            <tr
                            class="advads-conditions-connector advads-conditions-connector-<?php echo $connector; ?>">
                            <td colspan="3"><?php
								echo Advanced_Ads_Visitor_Conditions::render_connector_option( $i, $connector );
								?></td></tr>
						<?php endif; ?>
                        <tr>
                        <td class="advads-conditions-type"><?php echo $visitor_conditions[ $_options['type'] ]['label']; ?></td>
                        <td><?php
        ...
        <input type="hidden" id="advads-visitor-conditions-index"
               value="<?php echo isset( $options ) ? count( $options ) : 0; ?>"/>
    </div>
...
elseif ( Advanced_Ads_Checks::cache() && ! defined( 'AAP_VERSION' ) ) :
	?><p><span
        class="advads-error-message"><?php _e( 'It seems that a caching plugin is activated.', 'advanced-ads' ); ?></span>
    &nbsp;<?php
...
if ( isset( $options['mobile'] ) && '' !== $options['mobile'] ) :
	?><p
        style="color: red;"><?php _e( 'The visitor conditions below are deprecated. Please use the new version of visitor conditions to replace it.', 'advanced-ads' ); ?></p>
