...
<div class="wrap">
	<?php if ( isset( $_GET['message'] ) ) :
		if ( $_GET['message'] === 'error' ) :
            ...
            <p><?php _e( 'Couldn’t create the new placement. Please check your form field and whether the name is already in use.', 'advanced-ads' ); ?></p>
            </div><?php
	...
	<?php if ( isset( $placements ) && is_array( $placements ) && count( $placements ) ) :
        ...
        <form method="POST" action="">
				...
				foreach ( $placements as $_placement_slug => $_placement ) :
                    ...
                    <td><?php
						if ( $type_missing ) :  // type is not given
							?><p
                                class="advads-error-message"><?php printf( __( 'Placement type "%s" is missing and was reset to "default".<br/>Please check if the responsible add-on is activated.', 'advanced-ads' ), $missed_type ); ?></p><?php
						...
						<?php if ( ! isset( $_placement['type'] ) || 'default' === $_placement['type'] ) :
                            ...
                            <label><?php _e( 'shortcode', 'advanced-ads' ); ?>
                                <code><input type="text" onclick="this.select();"
                                             value='[the_ad_placement id="<?php echo $_placement_slug; ?>"]'/></code>
                            </label>
                            <label><?php _e( 'template', 'advanced-ads' ); ?>
                                <code><input type="text" onclick="this.select();"
                                             value="the_ad_placement('<?php echo $_placement_slug; ?>');"/></code>
                            </label>
                            </div><?php
						endif;
						?></td>
                    <td class="advads-placements-table-options">
                        ...
                        <select id="adsads-placements-item-<?php echo $_placement_slug; ?>"
                                name="advads[placements][<?php echo $_placement_slug; ?>][item]">
							...
							<?php if ( isset( $items['groups'] ) ) : ?>
                                <optgroup label="<?php _e( 'Ad Groups', 'advanced-ads' ); ?>">
									<?php foreach ( $items['groups'] as $_item_id => $_item_title ) : ?>
                                        <option value="<?php echo $_item_id; ?>" <?php if ( isset( $_placement['item'] ) ) {
						...
						switch ( $_placement['type'] ) :
							case 'post_content' :
								...
								$_positions = array( 'after'  => __( 'after', 'advanced-ads' ),
                                ...
                                <select name="advads[placements][<?php echo $_placement_slug; ?>][options][position]">
									<?php foreach ( $_positions as $_pos_key => $_pos ) : ?>
                                        <option value="<?php echo $_pos_key; ?>" <?php if ( isset( $_placement['options']['position'] ) ) {
                                ...
                                <input type="number"
                                       name="advads[placements][<?php echo $_placement_slug; ?>][options][index]"
                                       value="<?php
                                ...
                                <select name="advads[placements][<?php echo $_placement_slug; ?>][options][tag]">
									<?php foreach ( $tags as $_tag_key => $_tag ) : ?>
                                        <option value="<?php echo $_tag_key; ?>" <?php if ( isset( $_placement['options']['tag'] ) ) {
                                ...
                                <p><label><input type="checkbox"
                                                 name="advads[placements][<?php echo $_placement_slug; ?>][options][start_from_bottom]"
                                                 value="1" <?php
                                ...
                                <p><span class="advads-error-message"><?php _e( 'Important Notice', 'advanced-ads' ); ?>
                                        : </span><?php _e( 'Your server is missing an extension. This might break the content injection.<br/>Ignore this warning if everything works fine or else ask your hosting provider to enable <em>mbstring</em>.', 'advanced-ads' ); ?>
                                </p>
							<?php endif;
								break;
						endswitch;
						...
						if ( $advanced_options ) :
							?><a class="advads-toggle-link"
                                 onclick="advads_toggle('.advads-placements-advanced-options-<?php
							     echo $_placement_slug; ?>')">- <?php _e( 'advanced options', 'advanced-ads' ); ?> –</a>
                            <div class="advads-placements-advanced-options-<?php echo $_placement_slug; ?>"
                    ...
                    <td>
                        <input type="checkbox" id="adsads-placements-item-delete-<?php echo $_placement_slug; ?>"
                               name="advads[placements][<?php echo $_placement_slug; ?>][delete]" value="1"/>
            ...
            <input type="submit" id="advads-save-placements-button" class="button button-primary"
                   value="<?php _e( 'Save Placements', 'advanced-ads' ); ?>"/>
			<?php wp_nonce_field( 'advads-placement', 'advads_placement', true ) ?>
            <button type="button" title="<?php _e( 'Create a new placement', 'advanced-ads' ); ?>"
                    class="button-secondary" onclick="advads_toggle('.advads-placements-new-form')"><?php
	...
	?>
    <form method="POST" action="" onsubmit="return advads_validate_placement_form();"
          class="advads-placements-new-form"<?php if ( isset( $placements ) && count( $placements ) ) {
        ...
        <input type="submit" class="button button-primary"
               value="<?php _e( 'Save New Placement', 'advanced-ads' ); ?>"/>
    </form>
</div>
