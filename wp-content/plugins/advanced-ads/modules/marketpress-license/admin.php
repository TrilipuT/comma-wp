...
if ( class_exists( 'Advanced_Ads', false ) ) {
	...
	function render_settings_marketpress_license_callback() {
		...
		?>
        <style>
            div#licenses table tr:first-of-type td span.advads-license-activate-error a {
                display: none;
            }
        </style>
		<?php
		...
		if ( $license_key === '' ) :
            ...
            <td>
            <a class="advads-licenses-marketpress-show"
               href="javascript:void(0)"><?php echo __( 'Click here if you purchased a Bundle key through <strong>MarketPress</strong>.', 'advanced-ads' ); ?></a>
            <script>jQuery('.advads-licenses-marketpress-show').parents('td').prev('td').hide().prev('th').css('text-indent', '-9999px');
                jQuery('.advads-licenses-marketpress-show').click(function () {
                    jQuery(this).parents('td').hide().prev('td').show().prev('th').css('text-indent', '0').show();
                });</script>
		<?php endif;
